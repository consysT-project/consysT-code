package org.conloc.invariants.lib.examples.consensus;
...
class Consensus {
  ...
  public Consensus(int id) {
    if (!(id >= 0 && id < InvariantUtils.numOfReplicas()))
      throw new IllegalArgumentException("id not in range.");
    flag = false;
    replicaId = id;
    b = new boolean[InvariantUtils.numOfReplicas()];
    for(int i = 0; i < InvariantUtils.numOfReplicas(); ++i)
      b[i] = false;
  }
