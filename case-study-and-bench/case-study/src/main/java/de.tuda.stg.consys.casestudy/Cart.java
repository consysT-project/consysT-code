...
public class Cart implements Serializable {
    ...
    public boolean add(JRef<@Strong Product> item, int count, JReplicaSystem system) throws Error{
        ...
        for(int x = 0;x < count;x++){
            cartContent.invoke("append", item, system);
            //TODO: Why does inputing the system to cartcontent (jdistlist) not cause serialiation errors, but add does.
        }
        return true;
    }

    public boolean addNode(JRef<@Weak DistNode> node) throws Error {
        cartContent.invoke("appendNode", node);
        return true;
    }
    ...
    public boolean checkout(JRef<@Strong User> user, boolean PrintReceipt, String systemInfo){
        ...
        System.out.println("Cart content" + len);
