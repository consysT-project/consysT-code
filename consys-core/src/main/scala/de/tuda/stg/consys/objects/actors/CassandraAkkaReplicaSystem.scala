...
import de.tuda.stg.consys.objects.ConsistencyLevel.Cassandra
import de.tuda.stg.consys.objects.actors.Requests.{AsynchronousRequest, Operation, Request}
import de.tuda.stg.consys.objects.{ConsistencyLevel, actors}
...
trait CassandraAkkaReplicaSystem extends AkkaReplicaSystem {
	...
	override protected def createMasterReplica[T <: AnyRef : TypeTag](l : ConsistencyLevel, addr : Addr, obj : T) : AkkaReplicatedObject[Addr, T] = l match {
		case Cassandra(_) => new actors.CassandraAkkaReplicaSystem.CassandraReplicatedObject(obj, addr, this, l)
